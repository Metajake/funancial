...
getMonthlyDates = function(amountType, newDateObj){ // newDateObj only gets sent if the Calendar switches month
    let reoccurringDates = Amounts.find({type:amountType, reoccurring: true});
    let nonReoccurringDates = Amounts.find({type:amountType, reoccurring: false});
    let dateInfo = [];
    let thisDate = new Date();
    reoccurringDates.forEach(function(date){
        if(!newDateObj){
            billDate = thisDate.setDate(date.date);
        }else{
            billDate = thisDate.setFullYear(newDateObj.year, newDateObj.month - 1, date.date);
        }
        dateInfo.push({title : date.name+' '+date.amount, start : billDate })
    });
    nonReoccurringDates.forEach(function(date){
        dateInfo.push({title : date.name+' '+date.amount, start : date.date })
    });
    return dateInfo
};
